...
class LoadFile(APIView):
    ...
    def get(self, request, format=None):
        ...
        file_path = os.path.join(STATICFILES_DIRS[0], './data/factors.json')
        whole_dataset_df = pd.read_json(open(file_path, 'rU'))

        # return Response(file_path)

        return Response(whole_dataset_json.to_json(orient='index'))
