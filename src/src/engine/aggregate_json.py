...
#         self.column = ['ZONE','PERIOD', 'TEAM']
	...
	def readFactorJSON(self, base_cnt=10, domain = "", ndims = 3):
		...
		self.domain = domain
		self.ndims = ndims
		file = "{}_factors_3_{}_sample_fit.json".format(self.domain, self.ndims, self.base_cnt)
		with open(file) as f:
	...
	def readMetricJSON(self, base_cnt=10, domain = "", ndims = 3):
		...
		self.domain = domain
		self.ndims = ndims
		file = "{}_factors_{}_{}_sample_fit_metrics.json".format(self.domain, self.ndims, self.base_cnt)
		with open(file) as f:
			metrics = json.load(f)
		return metrics
