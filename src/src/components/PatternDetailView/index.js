...
class PatternDetailView extends Component {
	...
	render() {
		...
		console.log(data);
		const factors = this.state.factors[0].factors[0].values;
		// factors = factors[0]
		console.log(factors);
		...
		var x0 = d3.scaleBand()
		  ...
		  var keys_ = Object.keys(factors).sort();
		  console.log(keys_)

		  x0.domain(data.map(function(d) { return d.State; }));
		  x0.domain(keys_);
		  // x1.domain(keys).rangeRound([0, x0.bandwidth()]);
		  y.domain([0, d3.max(data, function(d) { return d3.max(keys_, function(key) { return d[key]; }); })]).nice();
		  ...
		  const rect_plot = g.selectAll(".detailView_col")
			...
			.attr("transform", function(d) { console.log(d); return "translate(" + x0(d.State) + ",0)"; });
