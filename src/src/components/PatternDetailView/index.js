  ...
  //   }
	...
	render() {
		...
		for(var i = 0; i < descriptor_size; i++){
			draw_bar_plot(svg, data, i, selectedPatterns, margin, width, height/descriptor_size);	
		}

    	function draw_bar_plot(svg, data, i, selectedPatterns, margin, width, height){
    		// temporarily showing 3 patterns
    		let data_ = data[i].slice(0,3);
      ...
      <div className={styles.PatternOverview}>
        <div className={index.title}>Pattern Detail View</div>
				<div>{'ddddddd' + this.props.selectedPatterns}</div>
				{svg.toReact()}
      </div>
	  );
